...
import "./interfaces/IMineTimelock.sol";
...
contract MineTimelock is IMineTimelock {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "MineTimelock: forbidden");
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "MineTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "MineTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "MineTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setExternalAdmin(address _target, address _admin) external onlyAdmin {
        require(_target != address(this), "MineTimelock: invalid _target");
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "MineTimelock: invalid _buffer");
        require(_buffer > buffer, "MineTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }
    ...
    function _mint(
        ...
        require(IERC20(_token).totalSupply() <= maxTokenSupply, "MineTimelock: maxTokenSupply exceeded");
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action] != 0, "MineTimelock: action not signalled");
        require(pendingActions[_action] < block.timestamp, "MineTimelock: action time not yet passed");
    }

    function _clearAction(bytes32 _action) private {
        require(pendingActions[_action] != 0, "MineTimelock: invalid _action");
