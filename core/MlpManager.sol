...
contract MlpManager is ReentrancyGuard, Governable, IMlpManager {
    ...
    function _addLiquidity(
        ...
        require(usdmAmount >= _minUsdm, "MlpManager: insufficient USDM output");

        uint256 mintAmount = aumInUsdm == 0 ? usdmAmount : usdmAmount.mul(mlpSupply).div(aumInUsdm);
        ...
        emit AddLiquidity(_account, _token, _amount, aumInUsdm, mlpSupply, usdmAmount, mintAmount);
    ...
    function _removeLiquidity(
        ...
        require(lastAddedAt[_account].add(cooldownDuration) <= block.timestamp, "MlpManager: cooldown duration not yet passed");
        ...
        emit RemoveLiquidity(_account, _tokenOut, _mlpAmount, aumInUsdm, mlpSupply, usdmAmount, amountOut);
